...
procedure RecUtiles;
function CheminOk(dep : TPoint; arv : integer) : boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (nextcell.position.Y = but) then
      begin
        stop := true;
        RecUtiles;
        Result := true;
...
var
  ...
  tmpcell: TAStarCell;
begin
  // On vérifi que la cellule ne sort pas de la carte et que l'on ne tombe
  // pas sur un mur
  // Il faut également que la cellule ne soit pas dans la liste fermée
  if (position.X >= 0) and (position.X < nbCases)
  and(position.Y >= 0) and (position.Y < nbCases)
  and(carte[position.X, position.Y] = 0)
...
//------------------------------------------------------------------------------
procedure TFAstar.InitImages;
begin
  murH := TBitmap.Create;
  murH.Width := 97;
  murH.Height := 16;
  murH.Transparent := true;
  murH.TransparentColor := clwhite;
  murH.Assign(Image2.Picture.Bitmap);
  murV := TBitmap.Create;
  murV.Width := 16;
  murV.Height := 97;
  murV.TransparentColor := clwhite;
  murV.Transparent := true;
  murV.Assign(Image3.Picture.Bitmap);
  murP := TBitmap.Create;
  murP.Width := 16;
  murP.Height := 97;
  murP.TransparentColor := clwhite;
  murP.Transparent := true;
  murP.Assign(Image4.Picture.Bitmap);
  vide := TBitmap.Create;
  vide.Width := 16;
  vide.Height := 97;
  vide.Assign(Image5.Picture.Bitmap);
end;

procedure TFAstar.FreeImages;
begin
  murH.Free;
  murV.Free;
  murP.Free;
